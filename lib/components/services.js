...
var component = function( opt, app ){
    ...
    this.service.filter( function( request, callback ){
        ...
        if( self.options.secure.indexOf(request.remoteAddress) === -1 ){
    ...
    this.service.filter(function( request, callback ){
        ...
        if( self.options.ban.indexOf(request.remoteAddress) !== -1 ){
            callback(new Error("You don`t have access, please contact the service administrator."));
            return;
        }
        callback(null,request);
    ...
    /// 预加载post方法
    try{
        var files = fs.readdirSync( this.app.base + "/app/services/"+ this.options.name+"/post/" );
        ...
        this.app.logger.warn("预加载post服务出错:"+exception);
    }
    ...
    try{
        ...
        this.app.logger.warn("预加载get服务出错:"+exception);
    }
}
